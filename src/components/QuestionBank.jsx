...
export default function QuestionBank() {
  ...
  // The three options are restored perfectly
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      // 🌟 THE INTERCEPTOR: Force the backend to generate 100% AI if Hybrid is selected
      ...
      const formatted = newQuestions.map(q => ({ 
        ...
        isAiGenerated: backendSourceType === 'AI' // Accurate tagging
      }));
    ...
    <Box sx={{ animation: "fadeIn 0.5s ease", pb: 10, color: "#fff", minHeight: "100vh" }}>
      ...
      {activeTab === 0 && (
        <Grid container spacing={3} sx={{ flexGrow: 1 }}>
          {/* LAYOUT UPGRADE: Slimmed down the settings panel from 26% to 18% width */}
          <Grid size={{ xs: 12, md: 2.2 }} sx={{ width: { xs: '100%', md: '18.33%' } }}>
            <Stack spacing={3} sx={{ position: "sticky", top: 20 }}>
              <Card sx={{ p: 2.5, bgcolor: "rgba(0, 221, 179, 0.02)", border: "1px solid rgba(0, 221, 179, 0.15)", borderRadius: "24px" }}>
                <Typography variant="subtitle2" sx={{ color: "#00DDB3", fontWeight: 800, mb: 2, display: "flex", alignItems: "center", gap: 1, letterSpacing: "0.5px" }}><Database size={16} /> 1. UPLOAD</Typography>
              ...
              <Card sx={{ p: 2.5, bgcolor: "rgba(6, 182, 212, 0.02)", borderRadius: "24px", border: "1px solid rgba(6, 182, 212, 0.15)" }}>
                <Typography variant="subtitle2" sx={{ color: "#06B6D4", fontWeight: 800, mb: 3, display: "flex", alignItems: "center", gap: 1, letterSpacing: "0.5px" }}><SlidersHorizontal size={16} /> 2. SETTINGS</Typography>
                ...
                <FormControl fullWidth variant="filled" sx={{ mb: 2.5, "& .MuiFilledInput-root": { bgcolor: "rgba(0,0,0,0.3)", borderRadius: "12px", color: "#fff", border: "1px solid rgba(255,255,255,0.05)" } }}>
                  <Select value={sourceType} onChange={(e) => setSourceType(e.target.value)} disableUnderline sx={{ fontWeight: 600, fontSize: "0.9rem" }}>
                    <MenuItem value="AI">AI Only (Locked)</MenuItem>
                    <MenuItem value="MANUAL">Manual Blank Canvas</MenuItem>
                    <MenuItem value="HYBRID">Hybrid (AI + Edits)</MenuItem>
                  </Select>
                </FormControl>

                <Grid container spacing={1.5} sx={{ mb: 2.5 }}>
                  <Grid size={{ xs: 6 }} sx={{ width: '50%' }}>
                ...
                <AnimatePresence>
                  {sourceType !== 'MANUAL' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}>
                      <Typography variant="caption" sx={{ color: "rgba(255,255,255,0.5)", mb: 1, display: "block", ml: 1 }}>AI Instructions</Typography>
          ...
          {/* LAYOUT UPGRADE: Vastly expanded the main workspace to 81% width! */}
          <Grid size={{ xs: 12, md: 9.8 }} sx={{ width: { xs: '100%', md: '81.66%' } }}>
            ...
            <Grid container spacing={3}>
              {/* LAYOUT UPGRADE: Slimmed down the outline nav so Editor gets maximum space */}
              <Grid size={{ xs: 3.5 }} sx={{ width: '29.16%' }}>
                <Card sx={{ p: 2, bgcolor: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "24px", height: "calc(100vh - 270px)", display: "flex", flexDirection: "column" }}>
                  ...
                  <List sx={{ flexGrow: 1, overflowY: "auto", px: 0 }}>
                    {(workspaceMode === "sandbox" ? aiDrafts : officialBank).map((q, idx) => (
                      <ListItemButton key={q.id} selected={activeQuestionId === q.id} onClick={() => setActiveQuestionId(q.id)} sx={{ borderRadius: "16px", mb: 1, p: 1.5, transition: "0.2s", "&.Mui-selected": { bgcolor: workspaceMode === "sandbox" ? "rgba(6, 182, 212, 0.1)" : "rgba(0, 221, 179, 0.1)", border: `1px solid ${workspaceMode === "sandbox" ? "rgba(6, 182, 212, 0.3)" : "rgba(0, 221, 179, 0.3)"}` }, "&:hover": { bgcolor: "rgba(255,255,255,0.03)" } }}>
                        ...
                        {q.correct === null && <Tooltip title="Missing Answer Key"><AlertCircle size={16} color="#f59e0b" /></Tooltip>}
                      </ListItemButton>
                    ))}
              ...
              {/* LAYOUT UPGRADE: Gave the Active Editor maximum screen real estate (over 70% width!) */}
              <Grid size={{ xs: 8.5 }} sx={{ width: '70.83%' }}>
                {activeQuestion ? (
                      ...
                      <Card sx={{ p: 5, bgcolor: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "24px", height: "calc(100vh - 270px)", overflowY: "auto" }}>
                        <Stack direction="row" justifyContent="space-between" mb={4} alignItems="center">
                          ...
                          <Stack direction="row" spacing={1.5}>
                            ...
                            {workspaceMode === "official" && <IconButton color="error" onClick={() => deleteFromBank(activeQuestion.id)} sx={{ opacity: 0.8, ml: 1, bgcolor: "rgba(255,0,0,0.1)", "&:hover": { bgcolor: "rgba(255,0,0,0.2)" } }}><Trash2 size={18} /></IconButton>}
                          </Stack>
                        </Stack>
                        ...
                        <Grid container spacing={3} sx={{ mb: 5 }}>
                          {activeQuestion.options.map((opt, oIdx) => {
                            ...
                            return (
                              <Grid size={{ xs: 12, sm: 6 }} sx={{ width: { xs: '100%', sm: '50%' } }} key={oIdx}>
        ...
        <Box sx={{ maxWidth: 800, mx: "auto", mt: 4 }}>
          <Card sx={{ p: 6, borderRadius: "24px", bgcolor: "rgba(255,255,255,0.02)", border: "1px solid rgba(0, 221, 179, 0.2)", textAlign: 'center' }}>
            ...
            <Grid container spacing={4} sx={{ mt: 2, textAlign: "left" }}>
              <Grid size={{ xs: 12, sm: 4 }} sx={{ width: { xs: '100%', sm: '33.33%' } }}><TextField fullWidth type="date" label="Execution Date" value={schedulingData.date} onChange={(e) => setSchedulingData({...schedulingData, date: e.target.value})} InputLabelProps={{ shrink: true }} sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)" } }} /></Grid>
              <Grid size={{ xs: 12, sm: 4 }} sx={{ width: { xs: '100%', sm: '33.33%' } }}><TextField fullWidth type="time" label="Launch Time" value={schedulingData.time} onChange={(e) => setSchedulingData({...schedulingData, time: e.target.value})} InputLabelProps={{ shrink: true }} sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)" } }} /></Grid>
              <Grid size={{ xs: 12, sm: 4 }} sx={{ width: { xs: '100%', sm: '33.33%' } }}><TextField fullWidth type="number" label="Duration (Mins)" value={schedulingData.duration} onChange={(e) => setSchedulingData({...schedulingData, duration: parseInt(e.target.value)})} InputLabelProps={{ shrink: true }} sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)" } }} /></Grid>
              <Grid size={{ xs: 12 }} sx={{ width: '100%' }}><TextField fullWidth label="Assigned Student Batch" value={schedulingData.batch} onChange={(e) => setSchedulingData({...schedulingData, batch: e.target.value})} placeholder="Ex: BSCS-2026-A" sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)" } }} /></Grid>
            </Grid>
                ...
                <Paper key={idx} sx={{ p: 3, bgcolor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "16px" }}>
                  ...
                  <Grid container spacing={1.5}>
                    {['A', 'B', 'C', 'D'].map((letter, optIdx) => {
                      ...
                      return (
                        <Grid size={{ xs: 12, sm: 6 }} sx={{ width: { xs: '100%', sm: '50%' } }} key={letter}>
