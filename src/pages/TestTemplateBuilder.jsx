...
export default function TestTemplateBuilder() {
  ...
  // --- MOCK ADMIN DATA ---
  // In a real app, you fetch this from the backend: "What courses is this teacher assigned to?"
  const assignedCourses = [
    "Software Architecture (CS-401)",
    "Data Structures & Algorithms (CS-201)",
    "Artificial Intelligence (CS-501)",
    "Database Management Systems (CS-304)"
  ];

  // --- 1. COURSE & META STATE ---
  const [selectedCourse, setSelectedCourse] = useState(assignedCourses[0]);
  const [templateName, setTemplateName] = useState("Midterm Exam Template");
  const [testCategory, setTestCategory] = useState("Midterm"); // Quiz, Midterm, Final, Practice
  ...
  const handleSaveTemplate = () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      alert(`"${templateName}" has been saved to ${selectedCourse} Templates!`);
    }, 1500);
  };
  ...
  return (
    <Box sx={{ animation: "fadeIn 0.5s ease", pb: 10, color: "#fff", minHeight: "100vh" }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 4 }}>
        ...
        <Button 
          variant="contained" onClick={handleSaveTemplate} disabled={isSaving}
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth variant="filled" sx={{ "& .MuiFilledInput-root": { bgcolor: "rgba(0,0,0,0.3)", borderRadius: "12px", border: "1px solid rgba(255,255,255,0.05)" } }}>
                    ...
                    <Select value={testCategory} onChange={(e) => setTestCategory(e.target.value)} disableUnderline sx={{ color: "#fff", fontWeight: 700 }}>
                ...
                <Grid item xs={12}>
                  ...
                  <TextField fullWidth variant="filled" value={templateName} onChange={(e) => setTemplateName(e.target.value)} InputProps={{ disableUnderline: true, sx: { borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)", color: "#fff", fontWeight: 700, border: "1px solid rgba(255,255,255,0.05)", fontSize: "1.1rem" } }} />
                </Grid>
              </Grid>
            </Card>
            ...
            <Card sx={{ p: 4, bgcolor: "rgba(255, 255, 255, 0.01)", border: "1px solid rgba(255, 255, 255, 0.08)", borderRadius: "24px" }}>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} sm={6}>
                  <Paper sx={{ p: 2.5, bgcolor: "rgba(0,0,0,0.3)", borderRadius: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Switch checked={behavior.shuffleQs} onChange={(e) => handleBehaviorChange("shuffleQs", e.target.checked)} sx={{ "& .MuiSwitch-switchBase.Mui-checked": { color: "#00DDB3" } }} />
                  </Paper>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Paper sx={{ p: 2.5, bgcolor: "rgba(0,0,0,0.3)", borderRadius: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Switch checked={behavior.preventTabSwitch} onChange={(e) => handleBehaviorChange("preventTabSwitch", e.target.checked)} color="warning" />
                  </Paper>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Paper sx={{ p: 2.5, bgcolor: "rgba(0,0,0,0.3)", borderRadius: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Switch checked={behavior.strictProctoring} onChange={(e) => handleBehaviorChange("strictProctoring", e.target.checked)} color="secondary" />
                  </Paper>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Paper sx={{ p: 2.5, bgcolor: "rgba(0,0,0,0.3)", borderRadius: "16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Switch checked={behavior.allowReview} onChange={(e) => handleBehaviorChange("allowReview", e.target.checked)} sx={{ "& .MuiSwitch-switchBase.Mui-checked": { color: "#06B6D4" } }} />
                  </Paper>
                </Grid>
              </Grid>
              ...
              <Grid container spacing={3} sx={{ mt: 1 }}>
                <Grid item xs={12} sm={6}>
                  ...
                  <Select fullWidth variant="filled" value={behavior.maxAttempts} onChange={(e) => handleBehaviorChange("maxAttempts", e.target.value)} disableUnderline sx={{ borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)", color: "#fff", fontWeight: 600, border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Select fullWidth variant="filled" value={behavior.showResults} onChange={(e) => handleBehaviorChange("showResults", e.target.value)} disableUnderline sx={{ borderRadius: "12px", bgcolor: "rgba(0,0,0,0.3)", color: "#fff", fontWeight: 600, border: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <Grid item xs={12} md={4}>
          <Box sx={{ position: "sticky", top: 20 }}>
            <Card sx={{ p: 0, bgcolor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "24px", overflow: "hidden" }}>
              ...
              <Box sx={{ p: 3 }}>
                {(() => {
                  const totalQs = hasSections ? sections.reduce((acc, curr) => acc + (parseInt(curr.count) || 0), 0) : "N/A";
                  const totalMarks = hasSections ? totalQs * marksPerQ : "N/A";
                  ...
                  return (
                    <Stack spacing={2.5}>
                      ...
                      <Box display="flex" justifyContent="space-between">
                        ...
                        <Typography variant="subtitle2" sx={{ fontWeight: 800, color: "#00DDB3" }}>{hasSections ? `${totalMarks} Pts` : "Dynamic"}</Typography>
                      </Box>
                      ...
                      <Stack spacing={1.5}>
                        ...
                        <Box display="flex" alignItems="center" gap={1.5}>
                          ...
                          <Typography variant="body2">Attempts: {behavior.maxAttempts === 999 ? "Unlimited" : behavior.maxAttempts}</Typography>
                        </Box>
              ...
              <Box sx={{ p: 2, bgcolor: "rgba(0,0,0,0.3)", textAlign: "center" }}>
                <Typography variant="caption" sx={{ color: "rgba(255,255,255,0.4)" }}>Template will be linked to {selectedCourse}</Typography>
              </Box>
            </Card>
          </Box>
        </Grid>
